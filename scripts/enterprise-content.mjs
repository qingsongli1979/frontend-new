...
export const enterpriseZh = {
  overview: page({
    ...
    name: "企业服务",
    serviceType: "企业代理与数据采集服务",
    title: "企业代理与数据采集服务 | 123Proxy",
    description: "123Proxy 面向爬虫开发工程师和 AI 数据团队，提供定制代理池、公开数据采集服务与 7x24 企业支持。",
    eyebrow: "123PROXY ENTERPRISE",
    titleLines: ["企业代理资源与", "公开数据采集服务"],
    lead: "提供定制代理池、10Gbps+ 单项目带宽、公开数据采集交付和 7x24 企业服务受理。",
    primaryLabel: "提交项目需求",
    ...
    governance: {
      ...
      text: "企业项目提供 7x24 服务受理；响应目标、支持渠道和升级路径以方案或合同为准。代理与采集服务仅用于公开或已授权数据，不得绕过登录、验证码、付费墙或其他访问控制。",
      points: ["7x24 服务受理", "响应目标按合同", "公开或已授权数据", "禁止绕过访问控制"]
    },
    faqs: [
      ["什么时候需要企业服务？", "需要定制资源、项目级带宽、数据交付、专属支持或合同化验收时，建议进入企业服务。标准规格明确的需求可以直接购买代理套餐。"],
      ["企业服务等于托管爬虫吗？", "不等于。代理基础设施、客户自有采集系统和数据采集服务是三个不同边界，是否包含程序开发、解析、存储和运维需要逐项确认。"],
      ["7x24 是否代表所有问题立即解决？", "7x24 指企业服务受理。首次响应、技术接手、更新频率与恢复目标由服务等级和合同约定。"],
      ["如何开始评估？", "发送目标类型、地区、规模、技术栈、代表性样本和完成周期，先判断使用标准产品、定制代理池还是数据采集服务。"]
    ],
  ...
  proxy: page({
    ...
    faqs: [
      ...
      ["定制代理池是否包含下载器和数据清洗？", "默认不包含。定制代理池交付网络基础设施，下载器、解析和存储由客户系统负责；需要数据结果时应评估数据采集服务。"],
  ...
  data: page({
    ...
    name: "数据采集服务",
    serviceType: "公开数据采集服务",
    title: "企业数据采集服务 - 公开数据项目交付 | 123Proxy",
    description: "123Proxy 面向公开网页、媒体、代码和文档任务，提供需求拆解、采集实施、质量验收与项目交付。",
    eyebrow: "MANAGED DATA SCRAPING",
    titleLines: ["公开数据采集、解析", "与结构化交付"],
    lead: "按来源、字段、规模和更新频率实施采集，交付 JSON、CSV、文件对象或客户指定存储。",
    primaryLabel: "评估采集项目",
    secondaryLabel: "查看定制代理池",
    secondaryUrl: "custom-proxy-pool.html",
    aside: {
    ...
    sectionTitle: "公开数据采集、解析与质量交付",
    sectionText: "项目可覆盖来源清单、采集程序、结构化解析、失败补采和约定格式交付。",
    offerings: [
      ["list-filter", "01", "需求与来源定义", "确认公开来源、字段、关系、规模、频率、截止时间和禁止范围。", null],
      ["braces", "02", "采集与解析实施", "围绕目标变化、失败重试、对象下载和结构化解析实施项目。", null],
      ["database", "03", "质量与数据交付", "按完整性、格式、来源记录和业务规则验收批次或增量数据。", null]
    ],
    process: {
      title: "数据项目交付流程",
      text: "确认来源与 Schema 后完成采集 PoC、质量基线和批次或增量交付。",
      steps: [
        ["01", "冻结来源与 Schema", "明确页面、对象、字段、关系、格式和更新方式。"],
        ["02", "建立采集 PoC", "用代表性样本验证访问、解析、媒体对象与失败场景。"],
    ...
    scope: {
      title: "交付范围与项目输入",
      text: "123Proxy 实施约定范围内的采集与交付；客户提供合法用途、字段样本、质量规则和接收方式。",
      includedLabel: "项目可包含",
      included: ["公开来源清单与采集策略", "采集程序、解析规则与失败重试", "文件对象或结构化数据交付", "质量报告、来源记录与约定补采"],
      inputLabel: "客户需要提供",
    ...
    matrix: {
      ...
      rows: [
        ...
        ["可追溯", "来源、采集时间和任务批次", "来源与版本记录"],
        ["时效性", "批次完成时间或增量延迟", "交付时间与变更说明"]
      ]
    },
    governance: {
      title: "公开或已授权数据",
      text: "数据采集服务不得用于绕过登录、验证码、付费墙、技术访问控制或目标方明确禁止的访问。项目启动前需要确认来源、用途、必要字段和数据保留策略。",
      points: ["公开或已授权来源", "目的与范围限定", "最小必要字段", "不绕过访问控制"]
    },
    faqs: [
      ["数据采集服务与代理产品有什么区别？", "代理产品提供网络出口；数据采集服务围绕来源、字段、程序、质量和交付结果实施项目。客户已有成熟采集系统时，通常只需要标准代理产品或定制代理池。"],
      ["可以采集哪些数据？", "仅限依法可访问的公开数据或客户已获授权的数据，具体来源和用途需在项目评估中确认。"],
      ["可以交付哪些格式？", "常见形式包括 JSON、CSV、文件对象或客户指定的存储路径，最终格式、Schema 和批次方式以项目范围为准。"],
      ["如何验收数据质量？", "通过代表性样本定义完整性、准确性、唯一性、时效性和来源追踪规则，并在 PoC 与正式方案中冻结。"]
    ],
    cta: ["获取数据采集与交付方案", "提交公开来源、字段样本、规模、频率、格式和完成周期。"]
  })
};
...
const labels = {
  "zh-CN": {
    home: "首页",
    category: "企业服务",
    nav: [["enterprise.html", "企业服务总览"], ["custom-proxy-pool.html", "定制代理池"], ["data-scraping-service.html", "数据采集服务"]],
